//Программа 1 для илюстрации работы с разделяемой памятью
...
int main(){
	...
	if ((array = (int *)shmat(shmid, NULL, 0)) == (int *)(-1)) {
		printf("Не можеиь присоединить область разделяемой памяти к адресному пространству текущего процесса ");
	}
        if(new){
		array[0] = 1;
		array[1] = 0;
